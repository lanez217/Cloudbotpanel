...
async function startBot(userId, phone, io, socket) {
    ...
    sock.ev.on('connection.update', async (update) => {
        ...
        if (!sock.authState.creds.registered && cleanPhone && !codeRequested) {
            codeRequested = true;
            socket.emit('status', 'Requesting pairing code from WhatsApp...');
            ...
            setTimeout(async () => {
                try {
                    ...
                    socket.emit('status', 'Failed to generate code. Please try again.');
                }
            }, 6000);
        }
    ...
    // Populate additional tools to reach 100+ commands
    for (let i = 1; i <= 35; i++) {
